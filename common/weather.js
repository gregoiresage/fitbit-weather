...
export default class Weather {
      ...
      if(utils.isRunningOnDevice()) {
        if (evt.data !== undefined && evt.data[WEATHER_MESSAGE_KEY] !== undefined) {
          ...
          if(evt.data[WEATHER_MESSAGE_KEY].error !== undefined){
            ...
            this._save(evt.data[WEATHER_MESSAGE_KEY]);
        ...
        if (evt.data !== undefined && evt.data[WEATHER_MESSAGE_KEY] !== undefined) {
          ...
          this._fetchRemote(message.provider, message.apiKey, message.feelsLike);
        }
      }
    });
  }
  ...
  _queryOWMWeather(apiKey, latitude, longitude, success, error) {
    ...
    .then((data) => { 
        ...
        if(success) success(this.weather);
    })
    .catch((err) => { if(error) error(err); });
  };

  _queryWUWeather(apiKey, feelsLike, latitude, longitude, success, error) {
  ...
  _queryDarkskyWeather(apiKey, feelsLike, latitude, longitude, success, error) {
  ...
  _queryYahooWeather(latitude, longitude, success, error) {
    ...
    .then((response) => {
      ...
      .then((data) => {
        ...
        var sunrise_time = Weather.timeParse(data.query.results.channel.astronomy.sunrise);
        var sunset_time  = Weather.timeParse(data.query.results.channel.astronomy.sunset);
        this.weather = {
  ...
  _fetchLocal() {
    geolocation.getCurrentPosition(
        (position) => {
          ...
          this._fetch(this._provider, this._apiKey, this._feelsLike, position.coords.latitude, position.coords.longitude, 
                (data) => {
                  this._save(data);
  ...
  _fetchRemote(provider, apiKey, feelsLike) {
    geolocation.getCurrentPosition(
        (position) => {
          this._fetch(provider, apiKey, feelsLike, position.coords.latitude, position.coords.longitude,
              (data) => {
                if (messaging.peerSocket.readyState === messaging.peerSocket.OPEN) {
                  let answer = {};
  ...
  _fetch(provider, apiKey, feelsLike, latitude, longitude, success, error) {
    // console.log("Latitude: " + latitude + " Longitude: " + longitude);
    if( provider === "owm" ) {
      this._queryOWMWeather(apiKey, latitude, longitude, success, error);
    }
    else if( provider === "wunderground" ) {
      this._queryWUWeather(apiKey, feelsLike, latitude, longitude, success, error);
    }
    else if( provider === "darksky" ) {
      this._queryDarkskyWeather(apiKey, feelsLike, latitude, longitude, success, error);
    }
    else 
    {
      this._queryYahooWeather(latitude, longitude, success, error);
    }
  }
